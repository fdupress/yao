...
(* IntDiv *)
module IntDiv = struct

  (* IntDiv.edivz *)
  let edivz : int -> int -> int * int =
    assert false (* TO FILL *)
  
  (* IntDiv.%/ *)
  let pcsl (m : int) (d : int) =
    (fun (x_0, x_1) -> x_0) (edivz m d)
...
module ArrayExt = struct
  ...
  (* ArrayExt.|| *)
  let brbr : 'x EcIArray.array0 -> 'x EcIArray.array0 -> 'x EcIArray.array0 =
    Array.append
  
  (* ArrayExt.mapi *)
  let mapi : (int -> 'x -> 'y) -> 'x EcIArray.array0 -> 'y EcIArray.array0 =
    Array.mapi

  (* ArrayExt.sub *)
  let sub : 'x EcIArray.array0 -> int -> int -> 'x EcIArray.array0 =
    Array.sub
    
  (* ArrayExt.take *)
  let take : int -> 'a EcIArray.array0 -> 'a EcIArray.array0 = fun l xs ->
    sub xs 0 l
  
  (* ArrayExt.drop *)
  let drop : int -> 'a EcIArray.array0 -> 'a EcIArray.array0 = fun l xs ->
    sub xs l (length xs - l)
  
  
  
  (* ArrayExt.blit *)
  let blit : 'x EcIArray.array0 ->
             int -> 'x EcIArray.array0 -> int -> int -> 'x EcIArray.array0 =
...
module Cyclic_group_prime = struct
  ...
  (* Cyclic_group_prime.^ *)
  let cf : group -> Prime_field.gf_q -> group = Lint.powm a e modulus
...
module Concrete = struct
  ...
  module ES = struct
    ...
    module SomeDKC = struct
      ...
      module KW = struct
        ...
        (* Concrete.ES.SomeDKC.KW.from_int *)
        let from_int : int -> word =
          assert false (* TO FILL *)
      
      end
      
      (* Concrete.ES.SomeDKC.w2kw *)
      let w2kw (w : W.word) =
        (if Pervasives.(=) w Option.witness then (Option.witness, W.getlsb w)
        else
          let kwi = IntDiv.pcsl (W.to_int w) 2 in
          let lsbi = W.getlsb w in (KW.from_int kwi, lsbi))
      ...
      module PrfDKC = struct
        ...
        let e (t : PRF.d) (k1 : W.word) (k2 : W.word) (m : W.word) =
          W.cf
            (W.cf (PRF.f (Pervasives.fst (w2kw k1)) t)
               (PRF.f (Pervasives.fst (w2kw k2)) t)) m
        
        (* Concrete.ES.SomeDKC.PrfDKC.D *)
        let d (t : PRF.d) (k1 : W.word) (k2 : W.word) (c : W.word) =
          W.cf
            (W.cf (PRF.f (Pervasives.fst (w2kw k1)) t)
               (PRF.f (Pervasives.fst (w2kw k2)) t)) c
    ...
    module Local = struct
      ...
      let init_dep (xs : 'x EcIArray.array0) (l : int)
        ...
        let r0 = ArrayExt.blit r 0 xs 0 (EcIArray.size xs) in
      ...
      let evalGen (f : 'a funct_t) (x : 'a EcIArray.array0)
        ...
        let (n, m, q, aa, bb) = (fun (x_0, x_1) -> x_0) f in
        ArrayExt.sub (evalComplete f x extract)
  ...
  let p1_stage2 (st : state1_t) (m3 : msg3_t) =
    ...
    let outG =
      ES.ProjScheme.Sch.Scheme.evalG fg_in (ArrayExt.brbr ot_out x2g_in) in
    ES.ProjScheme.Sch.Scheme.decode ko_in outG
  ...
  let p2_stage1 (i2 : ES.ProjScheme.Sch.Scheme.fun_t * bool EcIArray.array0)
    ...
    let x2g =
      ES.ProjScheme.Sch.Scheme.Input.encode
        (ArrayExt.drop
           (Pervasives.(+) n
              (Pervasives.(~-) (EcIArray.size (Pervasives.snd i2)))) ki) x2
    in
    let (st_ot, m1) =
      SomeOT.msg1
        (ArrayExt.take
           (Pervasives.(+) n
              (Pervasives.(~-) (EcIArray.size (Pervasives.snd i2)))) ki)
        (Pervasives.fst r_ot) in
    ((st_ot, fg, ko, x2g, Pervasives.snd r_ot), m1)
