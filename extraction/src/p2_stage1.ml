...
let tokensarray_to_string ts =
  let ls = Array.to_list (map (fun (x,y) -> intlist_to_string (string_to_intlist x) ^"*"^ intlist_to_string (string_to_intlist y)) ts) in
  String.concat ",," ls
...
let _ =
  ...
  let p2stage1inp = file_to_list "p2stage1.dat" in
