...
move => i1 r1 i2 r2.
 ...
 proof. 
 cut: ES.SG.SomeDKC.PrfDKC.Correct () by rewrite ES.SG.SomeDKC.PrfDKC_correct. 
 move => DKCh. 
...
lemma Security eps_ES eps_DDH eps_PRF : 
 ...
 proof.
   move => A1 A2 &m A1genll A1distll A2genll A2distll.

   cut ?: (forall i, 0 <= i < ES.SG.bound => 2%r * `|(Pr[ES.SG.SomeDKC.PRF.IND(ES.SG.SomeDKC.PRF.PRFr_Wrapped, ES.SG.SomeDKC.D(ES.SG.DKC_Adv(ES.SG.Sec.EncSecurity.RedSI(ES.Red(CSFE.B_G(SomeOT.S, SFE_A1(A1))))))).main(i)@ &m:res] - Pr[ES.SG.SomeDKC.PRF.IND(ES.SG.SomeDKC.PRF.RandomFunction, ES.SG.SomeDKC.D(ES.SG.DKC_Adv(ES.SG.Sec.EncSecurity.RedSI(ES.Red(CSFE.B_G(SomeOT.S, SFE_A1(A1))))))).main(i)@ &m:res])| <= eps_PRF) => (2%r * `|Mrplus.sum (fun i, (Pr[ES.SG.SomeDKC.PRF.IND(ES.SG.SomeDKC.PRF.PRFr_Wrapped, ES.SG.SomeDKC.D(ES.SG.DKC_Adv(ES.SG.Sec.EncSecurity.RedSI(ES.Red(CSFE.B_G(SomeOT.S, SFE_A1(A1))))))).main(i)@ &m:res] - Pr[ES.SG.SomeDKC.PRF.IND(ES.SG.SomeDKC.PRF.RandomFunction, ES.SG.SomeDKC.D(ES.SG.DKC_Adv(ES.SG.Sec.EncSecurity.RedSI(ES.Red(CSFE.B_G(SomeOT.S, SFE_A1(A1))))))).main(i)@ &m:res])) (intval 0 (ES.SG.bound-1))| <= ES.SG.bound%r * eps_PRF). 

   have : 0 <= ES.SG.bound by smt. 
   elim/intind ES.SG.bound.
   simplify intval. rewrite List.Iota.iota0; first by done. rewrite -set0E Mrplus.sum_empty => /#. 
   move => bound leq0_bound h_ind. 
   cut ->: bound + 1 - 1 = bound by algebra. 
   cut ->: (bound + 1)%r * eps_PRF = bound%r * eps_PRF + eps_PRF by idtac=>/#.
   move => h.

 
   rewrite (Mrplus.sum_rm _ _ bound). rewrite intval_def => /#. 
   simplify. 
   cut ->: (intval 0 bound `\` fset1 bound) = intval 0 (bound-1). rewrite fsetP => x. rewrite in_fsetD in_fset1 !intval_def => /#.
   pose pr_bound := Pr[ES.SG.SomeDKC.PRF.IND(ES.SG.SomeDKC.PRF.PRFr_Wrapped, ES.SG.SomeDKC.D(ES.SG.DKC_Adv(ES.SG.Sec.EncSecurity.RedSI(ES.Red(CSFE.B_G(SomeOT.S, SFE_A1(A1))))))).main
     (bound) @ &m : res] -
  Pr[ES.SG.SomeDKC.PRF.IND(ES.SG.SomeDKC.PRF.RandomFunction, ES.SG.SomeDKC.D(ES.SG.DKC_Adv(ES.SG.Sec.EncSecurity.RedSI(ES.Red(CSFE.B_G(SomeOT.S, SFE_A1(A1))))))).main
     (bound) @ &m : res].
 pose pr_sum := (Mrplus.sum
     (fun (i : int) =>
        Pr[ES.SG.SomeDKC.PRF.IND(ES.SG.SomeDKC.PRF.PRFr_Wrapped, ES.SG.SomeDKC.D(ES.SG.DKC_Adv(ES.SG.Sec.EncSecurity.RedSI(ES.Red(CSFE.B_G(SomeOT.S, SFE_A1(A1))))))).main
           (i) @ &m : res] -
        Pr[ES.SG.SomeDKC.PRF.IND(ES.SG.SomeDKC.PRF.RandomFunction, ES.SG.SomeDKC.D(ES.SG.DKC_Adv(ES.SG.Sec.EncSecurity.RedSI(ES.Red(CSFE.B_G(SomeOT.S, SFE_A1(A1))))))).main
           (i) @ &m : res]) (intval 0 (bound - 1))). 
     cut ->: 2%r * `|pr_bound + pr_sum| = `|2%r * pr_bound + 2%r * pr_sum| by idtac=>/#.
             simplify pr_bound pr_sum.
             cut ? : `|2%r *
(Pr[ES.SG.SomeDKC.PRF.IND(ES.SG.SomeDKC.PRF.PRFr_Wrapped, ES.SG.SomeDKC.D(ES.SG.DKC_Adv(ES.SG.Sec.EncSecurity.RedSI(ES.Red(CSFE.B_G(SomeOT.S, SFE_A1(A1))))))).main
    (bound) @ &m : res] -
 Pr[ES.SG.SomeDKC.PRF.IND(ES.SG.SomeDKC.PRF.RandomFunction, ES.SG.SomeDKC.D(ES.SG.DKC_Adv(ES.SG.Sec.EncSecurity.RedSI(ES.Red(CSFE.B_G(SomeOT.S, SFE_A1(A1))))))).main
    (bound) @ &m : res])| <= eps_PRF by idtac=>/#.
      idtac=>/#.
